...
{
	struct mock_alloc_args *args = mock_type(struct mock_alloc_args *);
...
{
	struct mock_dealloc_args *args = mock_type(struct mock_dealloc_args *);
	if (args->validate_params == MOCK_VALIDATE)
...
peer_new_test_alloc_pd_fail_ENOMEM(void **unused)
{
	 ...
	 * has failed.
	 */
	struct mock_alloc_args alloc_args = {MOCK_VALIDATE, NULL};
...
{
	/* configure mocks */
	struct mock_alloc_args alloc_args = {MOCK_PASSTHROUGH, MOCK_IBV_PD};
	will_return_maybe(ibv_alloc_pd, &alloc_args);
	struct mock_dealloc_args dealloc_args = {MOCK_PASSTHROUGH, NO_ERROR};
...
peer_new_test_success(void **unused)
{
	 ...
	 * succeeded.
	 */
	struct mock_alloc_args alloc_args = {MOCK_VALIDATE, MOCK_IBV_PD};
	will_return(ibv_alloc_pd, &alloc_args);
	expect_value(ibv_alloc_pd, ibv_ctx, MOCK_IBV_CTX);
	 ...
	 * rpma_peer_delete().
	 */
	struct mock_dealloc_args dealloc_args = {MOCK_VALIDATE, NO_ERROR};
...
peer_delete_test_dealloc_pd_fail(void **peer_ptr)
{
	 ...
	 * rpma_peer_delete().
	 */
	struct mock_dealloc_args dealloc_args = {MOCK_VALIDATE, EBUSY};
	will_return(ibv_dealloc_pd, &dealloc_args);
	expect_value(ibv_dealloc_pd, pd, MOCK_IBV_PD);
