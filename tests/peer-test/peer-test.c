...
{
	struct ibv_dealloc_pd_args *args =
			mock_type(struct ibv_dealloc_pd_args *);
	if (args->validate_params == MOCK_VALIDATE)
...
peer_new_test_malloc_fail(void **unused)
{
	...
	struct ibv_dealloc_pd_args dealloc_args = {MOCK_PASSTHROUGH, NO_ERROR};
...
peer_new_test_success(void **unused)
{
	 ...
	 * rpma_peer_delete().
	 */
	struct ibv_dealloc_pd_args dealloc_args = {MOCK_VALIDATE, NO_ERROR};
...
peer_delete_test_dealloc_pd_fail(void **peer_ptr)
{
	 ...
	 * rpma_peer_delete().
	 */
	struct ibv_dealloc_pd_args dealloc_args = {MOCK_VALIDATE, EBUSY};
	will_return(ibv_dealloc_pd, &dealloc_args);
	expect_value(ibv_dealloc_pd, pd, MOCK_IBV_PD);
