...
void
test_set_backtrace_level(void **unused)
{
	enum rpma_log_level level;
	for (level = RPMA_LOG_DISABLED; level <= RPMA_LOG_DEBUG; level ++) {
		assert_int_equal(0, rpma_log_set_backtrace_level(level));
		assert_int_equal(level, rpma_log_get_backtrace_level());
	}
}

void
test_set_backtrace_level_invalid(void **unused)
{
	enum rpma_log_level level = RPMA_LOG_DEBUG;
	level ++;
	assert_int_equal(RPMA_E_INVAL, rpma_log_set_backtrace_level(level));
	level = RPMA_LOG_DISABLED;
	level --;
	assert_int_equal(RPMA_E_INVAL, rpma_log_set_backtrace_level(level));
}
