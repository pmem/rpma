...
#define MOCK_WRITE_ADDR (&msg)
#define MOCK_WRITE_LEN (strlen(msg) + 1)
...
test_client__success(void **unused)
{
	...
	struct posix_memalign_args hellot = {0};
	will_return(__wrap_posix_memalign, &hellot);
	...
	will_return(__wrap_posix_memalign, &res1);
	...
	struct posix_memalign_args wr_allocated = {0};
	will_return(__wrap_posix_memalign, &wr_allocated);
	expect_value(ibv_reg_mr, pd, MOCK_IBV_PD);
	expect_value(ibv_reg_mr, length, sizeof(struct hello_t));
	expect_value(ibv_reg_mr, access, IBV_ACCESS_REMOTE_WRITE);
	will_return(ibv_reg_mr, &wr_allocated.ptr);
	...
	expect_value(ibv_post_send_mock, wr->sg_list->length, MOCK_WRITE_LEN);
...
{
	struct posix_memalign_args mr_allocated = {0};
	will_return(__wrap_posix_memalign, &mr_allocated);
	...
	f_event.param.conn.private_data_len = MOCK_WRITE_LEN * sizeof(char);
