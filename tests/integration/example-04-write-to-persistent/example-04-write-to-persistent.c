...
test_client__success(void **unused)
{
	...
	struct posix_memalign_args conn_raw = {0};
	will_return(__wrap_posix_memalign, &conn_raw);
	...
	will_return(ibv_reg_mr, &conn_raw.ptr);
	will_return(ibv_reg_mr, MOCK_MR_CONN);
	...
	expect_value(ibv_reg_mr, pd, MOCK_IBV_PD);
	expect_value(ibv_reg_mr, length, 1028);
