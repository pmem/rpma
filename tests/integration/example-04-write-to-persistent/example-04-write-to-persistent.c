...
#include "librpma.h"
#include "mocks.h"
#define RPMA_INTEGRATION_TEST
#include "hello.h"

#define MOCK_WRITE_ADDR (&wr_msg)
#define MOCK_READ_ADDR (&wr_msg)
#define MOCK_HELLO_SIZE HELLO_T_SIZE
...
test_client__success(void **unused)
{
	...
	expect_value(ibv_post_send_mock, wr->wr.rdma.remote_addr,
			(char *)MOCK_WRITE_ADDR + MOCK_DATA_OFFSET);
	will_return(ibv_post_send_mock, (uintptr_t)&mr_ptr.ptr);
	...
	/* deregister the memory region: flush buffer */
