...
/*
 * test_client -- 'read to volatile' integration test, client
 */
void
...
/*
 * test_server -- 'read to volatile' integration test, server
 */
void
test_server(void **unused)
{
	...
	expect_value(ibv_reg_mr, pd, MOCK_IBV_PD);
	expect_value(ibv_reg_mr, length, strlen(HELLO_STR) + 1);
