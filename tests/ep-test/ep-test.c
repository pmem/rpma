...
{
	/* configure mocks: */
	static struct ep_state estate = {0};
	will_return(rdma_create_event_channel, &estate.evch);
	expect_value(rdma_create_id, channel, &estate.evch);
	will_return(rdma_create_id, &estate.cmid);
	will_return(rpma_info_new, MOCK_INFO);
	expect_value(rpma_info_bind_addr, id, &estate.cmid);
	will_return(rpma_info_bind_addr, NO_ERROR);
	expect_value(rdma_listen, id, &estate.cmid);
	will_return(rdma_listen, NO_ERROR);
...
{
	struct ep_state *estate = *estate_ptr;
...
main(int argc, char *argv[])
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test_setup_teardown(ep_test_lifecycle, ep_setup, ep_teardown),
