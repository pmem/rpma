...
/*
 * set__cq_size_negative -- cq_size < 0 is invalid
 */
static void
set__cq_size_negative(void **cstate_ptr)
{
	struct conn_cfg_test_state *cstate = *cstate_ptr;

	/* cache the before value */
	int before;
	int ret = rpma_conn_cfg_get_cq_size(cstate->cfg, &before);
	assert_int_equal(ret, MOCK_OK);

	/* run test */
	ret = rpma_conn_cfg_set_cq_size(cstate->cfg, -1);

	/* verify the results */
	assert_int_equal(ret, RPMA_E_INVAL);
	int after;
	ret = rpma_conn_cfg_get_cq_size(cstate->cfg, &after);
	assert_int_equal(ret, MOCK_OK);
...
static const struct CMUnitTest test_connect[] = {
	...
	cmocka_unit_test_setup_teardown(set__cq_size_negative,
