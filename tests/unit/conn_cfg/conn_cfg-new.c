...
new__success(void **cstate_ptr)
{
	...
	ret = rpma_conn_cfg_get_cq_size(cstate->cfg, &a);
	assert_int_equal(ret, MOCK_OK);
	ret = rpma_conn_cfg_get_cq_size(cfg_default, &b);
	assert_int_equal(ret, MOCK_OK);
	assert_int_equal(a, b);

	ret = rpma_conn_cfg_get_sq_size(cstate->cfg, &a);
	assert_int_equal(ret, MOCK_OK);
	ret = rpma_conn_cfg_get_sq_size(cfg_default, &b);
	assert_int_equal(ret, MOCK_OK);
	assert_int_equal(a, b);

	ret = rpma_conn_cfg_get_rq_size(cstate->cfg, &a);
	assert_int_equal(ret, MOCK_OK);
	ret = rpma_conn_cfg_get_rq_size(cfg_default, &b);
	assert_int_equal(ret, MOCK_OK);
	assert_int_equal(a, b);
}
