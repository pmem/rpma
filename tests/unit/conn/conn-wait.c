...
wait__conn_NULL(void **unused)
{
	...
	int ret = rpma_conn_wait(NULL, &cq);
...
{
	/* run test */
	int ret = rpma_conn_wait(MOCK_CONN, NULL);
...
/*
 * wait__channel_NULL - NULL conn->channel is invalid
 */
static void
wait__channel_NULL(void **cstate_ptr)
{
	struct conn_test_state *cstate = *cstate_ptr;

	/* run test */
	struct rpma_cq *cq = NULL;
	int ret = rpma_conn_wait(cstate->conn, &cq);
...
wait__get_cq_event_UNKNOWN(void **cstate_ptr)
{
	...
	will_return(rpma_cq_get_ibv_cq, MOCK_ERRNO);
...
wait__success(void **cstate_ptr)
{
	...
	assert_ptr_equal(cq, MOCK_IBV_RCQ);
}
...
static const struct CMUnitTest tests_new[] = {
	...
	cmocka_unit_test_prestate_setup_teardown(wait__channel_NULL,
