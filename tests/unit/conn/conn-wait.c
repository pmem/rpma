...
/*
 * wait__is_rcq_NULL - NULL is_rcq is invalid
 */
static void
wait__is_rcq_NULL(void **unused)
{
	/* run test */
	struct rpma_cq *cq = NULL;
	int ret = rpma_conn_wait(MOCK_CONN, &cq, NULL);
...
static const struct CMUnitTest tests_new[] = {
	...
	cmocka_unit_test(wait__is_rcq_NULL),
	cmocka_unit_test_setup_teardown(wait__channel_NULL,
