...
/*
 * completion_get__cq_get_E_PROVIDER - rpma_cq_get()
 * fails with RPMA_E_PROVIDER
 */
static void
completion_get__cq_get_E_PROVIDER(void **cstate_ptr)
{
	struct conn_test_state *cstate = *cstate_ptr;

	will_return(rpma_cq_get, RPMA_E_PROVIDER);
...
completion_get__success(void **cstate_ptr)
{
	...
	assert_int_equal(cmpl.op, RPMA_OP_SEND);
}
...
static const struct CMUnitTest tests_completion_get[] = {
	...
	cmocka_unit_test_setup_teardown(
		completion_get__cq_get_E_PROVIDER,
		setup__conn_new, teardown__conn_delete),
	cmocka_unit_test_setup_teardown(
