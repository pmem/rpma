...
flush__success_FLUSH_TYPE_VISIBILITY(void **cstate_ptr)
{
	...
	/* configure mocks */
	expect_value(rpma_flush_mock_request, qp, MOCK_QP);
	expect_value(rpma_flush_mock_request, flush, MOCK_FLUSH);
	expect_value(rpma_flush_mock_request, dst, MOCK_RPMA_MR_REMOTE);
	expect_value(rpma_flush_mock_request, dst_offset, MOCK_REMOTE_OFFSET);
	expect_value(rpma_flush_mock_request, len, MOCK_LEN);
	expect_value(rpma_flush_mock_request, flags, MOCK_FLAGS);
	expect_value(rpma_flush_mock_request, op_context, MOCK_OP_CONTEXT);
