...
{
	/* run test */
	int ret = rpma_srq_new(&Srq_new_peer, NULL, NULL);
...
/*
 * new__ibv_create_srq_ERRNO -- ibv_create_srq() fails with MOCK_ERRNO
 */
static void
new__ibv_create_srq_ERRNO(void **cstate_ptr)
{
...
/*
 * new__rpma_cq_new_ERRNO -- rpma_cq_new() fails with MOCK_ERRNO
 */
static void
new__rpma_cq_new_ERRNO(void **cstate_ptr)
{
	struct srq_new_test_state *cstate = *cstate_ptr;
...
/*
 * new__rpma_cq_new_ERRNO_subsequent_ERRNO2 -- ibv_destroy_srq()
 * fails with MOCK_ERRNO2 after rpma_cq_new() failed with MOCK_ERRNO
 */
static void
new__rpma_cq_new_ERRNO_subsequent_ERRNO2(void **cstate_ptr)
{
	struct srq_new_test_state *cstate = *cstate_ptr;
...
static const struct CMUnitTest tests_new_delete[] = {
	...
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(new__ibv_create_srq_ERRNO),
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(new__rpma_cq_new_ERRNO),
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(
		new__rpma_cq_new_ERRNO_subsequent_ERRNO2),
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(new__malloc_ERRNO),
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(new__malloc_ERRNO_subsequent_ERRNO2),
	...
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(delete__rpma_cq_delete_ERRNO),
	SRQ_NEW_TEST_WITH_AND_WITHOUT_CFG(
