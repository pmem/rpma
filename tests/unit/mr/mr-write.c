...
/*
 * write__failed_E_PROVIDER_COMPL_ON_SUCCESS - rpma_mr_write failed
 * with RPMA_E_PROVIDER when send_flags == RPMA_F_COMPLETION_ON_SUCCESS
 */
static void
write__failed_E_PROVIDER_COMPL_ON_SUCCESS(void **mrs_ptr)
{
	...
	args.send_flags = RPMA_F_COMPLETION_ON_SUCCESS;
	...
	int ret = rpma_mr_write(MOCK_QP, mrs->remote, MOCK_DST_OFFSET,
				...
				MOCK_LEN, RPMA_F_COMPLETION_ON_SUCCESS,
				MOCK_OP_CONTEXT);
...
/*
 * write__failed_E_PROVIDER_COMPL_ON_ERROR - rpma_mr_write failed
 * with RPMA_E_PROVIDER when send_flags == 0 for RPMA_F_COMPLETION_ON_ERROR
 */
static void
write__failed_E_PROVIDER_COMPL_ON_ERROR(void **mrs_ptr)
{
	struct mrs *mrs = (struct mrs *)*mrs_ptr;
...
const struct CMUnitTest tests_mr_write[] = {
	...
	cmocka_unit_test_setup_teardown(
			write__failed_E_PROVIDER_COMPL_ON_SUCCESS,
			setup__mr_local_and_remote,
			teardown__mr_local_and_remote),
	cmocka_unit_test_setup_teardown(
			write__failed_E_PROVIDER_COMPL_ON_ERROR,
			setup__mr_local_and_remote,
			teardown__mr_local_and_remote),
