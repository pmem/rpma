...
static int types[] = {RPMA_FLUSH_TYPE_VISIBILITY, RPMA_FLUSH_TYPE_PERSISTENT};
...
flush__COMPL_ON_ERROR_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_types; i++) {
		/* configure mocks */
		configure_mr_flush(types[i], RPMA_F_COMPLETION_ON_ERROR, MOCK_ERRNO);

		/* run test */
		int ret = rpma_mr_flush(MOCK_QP, mrs->remote, MOCK_DST_OFFSET, MOCK_LEN,
				types[i], RPMA_F_COMPLETION_ON_ERROR, MOCK_OP_CONTEXT);
...
flush__COMPL_ON_SUCCESS_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_types; i++) {
		/* configure mocks */
		configure_mr_flush(types[i], RPMA_F_COMPLETION_ALWAYS, MOCK_ERRNO);

		/* run test */
		int ret = rpma_mr_flush(MOCK_QP, mrs->remote, MOCK_DST_OFFSET, MOCK_LEN,
				types[i], RPMA_F_COMPLETION_ALWAYS, MOCK_OP_CONTEXT);
...
flush__COMPLETION_ON_ERROR_success(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_types; i++) {
		/* configure mocks */
		configure_mr_flush(types[i], RPMA_F_COMPLETION_ON_ERROR, MOCK_OK);
 ...
 * RPMA_F_COMPLETION_ALWAYS is specified
 */
static void
flush__COMPLETION_ALWAYS_success(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_types; i++) {
		/* configure mocks */
		configure_mr_flush(types[i], RPMA_F_COMPLETION_ALWAYS, MOCK_OK);
