...
/*
 * configure_mr_atomic_write and configure_mr_atomic_write_with_qpx
 * -- configure common mock for rpma_mr_atomic_write()
 */
static void
...
#ifdef NATIVE_ATOMIC_WRITE_SUPPORTED
static void
configure_mr_atomic_write_with_qpx(struct ibv_qp_ex *qpx, int flags, int ret)
{
	/* configure mock */
	...
	if (qpx && qpx->wr_atomic_write) {
		...
		configure_mr_atomic_write(flags, ret);
	}
}
#endif
...
atomic_write__COMPL_ON_ERROR_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_qpxs; i++) {
		/* configure mocks */
		configure_mr_atomic_write_with_qpx(qpxs[i], RPMA_F_COMPLETION_ON_ERROR,
				MOCK_ERRNO);
...
atomic_write__COMPL_ON_SUCCESS_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_qpxs; i++) {
		/* configure mocks */
		configure_mr_atomic_write_with_qpx(qpxs[i], RPMA_F_COMPLETION_ALWAYS, MOCK_ERRNO);
...
atomic_write__COMPLETION_ALWAYS_success(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_qpxs; i++) {
		/* configure mocks */
		configure_mr_atomic_write_with_qpx(qpxs[i], RPMA_F_COMPLETION_ALWAYS, MOCK_OK);
...
atomic_write__COMPLETION_ON_ERROR_success(void **mrs_ptr)
{
	...
	for (int i = 0; i < num_qpxs; i++) {
		/* configure mocks */
		configure_mr_atomic_write_with_qpx(qpxs[i], RPMA_F_COMPLETION_ON_ERROR, MOCK_OK);
