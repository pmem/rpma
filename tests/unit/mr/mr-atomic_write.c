...
/*
 * atomic_write__failed_E_NOSUPP - rpma_mr_atomic_write
 * failed with RPMA_E_NOSUPP
 */
static void
atomic_write__failed_E_NOSUPP(void **mrs_ptr)
{
	struct mrs *mrs = (struct mrs *)*mrs_ptr;

	/* run test */
	int ret = rpma_mr_atomic_write(MOCK_QP, mrs->remote, MOCK_DST_OFFSET,
			Mock_src, MOCK_FLAGS, MOCK_OP_CONTEXT);
 ...
 * when send_flags == IBV_SEND_SIGNALED
...
atomic_write__COMPL_ON_ERROR_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	args.send_flags = IBV_SEND_SIGNALED; /* RPMA_F_COMPLETION_ON_ERROR */
 ...
 */
static void
atomic_write__success(void **mrs_ptr)
{
	struct mrs *mrs = (struct mrs *)*mrs_ptr;
...
static const struct CMUnitTest tests_mr__atomic_write[] = {
	...
	cmocka_unit_test_setup_teardown(atomic_write__failed_E_NOSUPP,
