...
atomic_cmp_swp__COMPL_ALWAYS_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	args.rkey = MOCK_RKEY;
	args.compare = MOCK_COMPARE;
	args.swap = MOCK_SWAP;
	args.ret = MOCK_ERRNO;
	will_return(ibv_post_send_mock, &args);

	/* run test */
	int ret = rpma_mr_atomic_cmp_swp(MOCK_QP,
			mrs->remote, MOCK_DST_OFFSET,
			mrs->local, MOCK_SRC_OFFSET,
			RPMA_F_COMPLETION_ALWAYS, MOCK_COMPARE,
...
atomic_cmp_swp__COMPL_ON_ERROR_failed_E_PROVIDER(void **mrs_ptr)
{
	...
	int ret = rpma_mr_atomic_cmp_swp(MOCK_QP,
			mrs->remote, MOCK_DST_OFFSET,
			mrs->local, MOCK_SRC_OFFSET,
			RPMA_F_COMPLETION_ON_ERROR, MOCK_COMPARE,
			MOCK_SWAP, MOCK_OP_CONTEXT);
