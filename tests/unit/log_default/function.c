...
setup_thresholds(void **config_ptr)
{
	...
	/*
	 * The primary log threshold should not affect logging function
	 * behaviour.
	 */
	Rpma_log_threshold[RPMA_LOG_THRESHOLD_PRIMARY] = RPMA_LOG_DISABLED;
	Rpma_log_threshold[RPMA_LOG_THRESHOLD_SECONDARY] = cfg->secondary;
...
static char *Strftim_ret = MOCK_TIME_OF_DAY_STR;
...
#define MOCK_GET_TIMESTAMP_CONFIGURE(x) \
	if ((x)->clock_gettime_error) { \
		...
		will_return(__wrap_localtime, &Timespec); \
		will_return(__wrap_localtime, &Tm); \
		will_return(__wrap_strftime, Strftim_ret); \
		will_return(__wrap_snprintf, MOCK_STDIO_ERROR); \
	} else { \
		will_return(__wrap_clock_gettime, &Timespec); \
		will_return(__wrap_localtime, &Timespec); \
