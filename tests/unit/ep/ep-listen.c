...
/*
 * ep-listen.c -- the endpoint unit tests of the following API:
 *
 * - rpma_ep_listen()
 * - rpma_ep_shutdown()
 */
...
/*
 * ep_listen__peer_NULL - NULL peer is invalid
 */
static void
ep_listen__peer_NULL(void **unused)
{
	/* run test */
...
/*
 * ep_listen__addr_NULL - NULL addr is invalid
 */
static void
ep_listen__addr_NULL(void **unused)
{
	/* run test */
	struct rpma_ep *ep = NULL;
...
/*
 * ep_listen__service_NULL - NULL service is invalid
 */
static void
ep_listen__service_NULL(void **unused)
{
	/* run test */
	struct rpma_ep *ep = NULL;
...
/*
 * ep_listen__ep_ptr_NULL - NULL ep_ptr is invalid
 */
static void
ep_listen__ep_ptr_NULL(void **unused)
{
	/* run test */
...
/*
 * ep_listen__peer_addr_service_ep_ptr_NULL - peer, addr, service
 * and ep_ptr == NULL are invalid
 */
static void
ep_listen__peer_addr_service_ep_ptr_NULL(void **unused)
{
	/* run test */
...
/*
 * ep_listen__create_evch_EAGAIN - rdma_create_event_channel() fails
 * with EAGAIN
 */
static void
ep_listen__create_evch_EAGAIN(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing:
...
/*
 * ep_listen__create_id_EAGAIN - rdma_create_id() fails with EAGAIN
 */
static void
ep_listen__create_id_EAGAIN(void **unused)
{
	/*
	 * configure mocks:
	 * - constructing
...
/*
 * ep_listen__info_new_E_NOMEM - rpma_info_new() fails with
 * RPMA_E_NOMEM
 */
static void
ep_listen__info_new_E_NOMEM(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing
...
/*
 * ep_listen__info_bind_addr_E_PROVIDER - rpma_info_bind_addr() fails
 * with RPMA_E_PROVIDER
 */
static void
ep_listen__info_bind_addr_E_PROVIDER(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing
...
/*
 * ep_listen__listen_EAGAIN - rdma_listen() fails with EAGAIN
 */
static void
ep_listen__listen_EAGAIN(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing
...
/*
 * ep_listen__malloc_ENOMEM - malloc() fails with ENOMEM
 */
static void
ep_listen__malloc_ENOMEM(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing
...
/*
 * ep_listen__malloc_ENOMEM_destroy_id_EAGAIN - malloc() fails with ENOMEM
 ...
 */
static void
ep_listen__malloc_ENOMEM_destroy_id_EAGAIN(void **unused)
{
	/*
	 * configure mocks for:
	 * - constructing
...
/*
 * ep_shutdown__ep_ptr_NULL - NULL ep_ptr is invalid
 */
static void
ep_shutdown__ep_ptr_NULL(void **unused)
{
	/* run test */
...
/*
 * ep_shutdown__ep_NULL - NULL ep is valid
 */
static void
ep_shutdown__ep_NULL(void **unused)
{
	/* run test */
	struct rpma_ep *ep = NULL;
...
/*
 * ep_shutdown__destroy_id_EAGAIN -- rdma_destroy_id() fails with EAGAIN
 */
static void
ep_shutdown__destroy_id_EAGAIN(void **estate_ptr)
{
...
main(int argc, char *argv[])
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test(ep_listen__peer_NULL),
		cmocka_unit_test(ep_listen__addr_NULL),
		cmocka_unit_test(ep_listen__service_NULL),
		cmocka_unit_test(ep_listen__ep_ptr_NULL),
		cmocka_unit_test(ep_listen__peer_addr_service_ep_ptr_NULL),
		cmocka_unit_test(ep_listen__create_evch_EAGAIN),
		cmocka_unit_test(ep_listen__create_id_EAGAIN),
		cmocka_unit_test(ep_listen__info_new_E_NOMEM),
		cmocka_unit_test(ep_listen__info_bind_addr_E_PROVIDER),
		cmocka_unit_test(ep_listen__listen_EAGAIN),
		cmocka_unit_test(ep_listen__malloc_ENOMEM),
		cmocka_unit_test(
			ep_listen__malloc_ENOMEM_destroy_id_EAGAIN),
		...
		cmocka_unit_test(ep_shutdown__ep_ptr_NULL),
		cmocka_unit_test(ep_shutdown__ep_NULL),
		cmocka_unit_test_setup_teardown(
			ep_shutdown__destroy_id_EAGAIN,
			setup__ep_listen, teardown__ep_shutdown),
