...
/*
 * init_fini_default -- default constructor and destructor initialized syslog
 * and set proper default thresholds
 * init function?
 */
extern void rpma_log_init_default(void);
extern void rpma_log_fini_default(void);

void
init_fini_default(void  **unused)
{
	/* we expectd syslog operations */
	syslog_mock_enable();
	...
	/* run tests */
	rpma_log_fini_default();
}

const struct CMUnitTest log_test_init_fini_default[] = {
	cmocka_unit_test(init_fini_default),
