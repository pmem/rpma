...
create_srq__ibv_create_srq_ERRNO(void **pprestate)
{
	...
	expect_value(ibv_create_srq, srq_init_attr->attr.max_wr,
			Create_srq_cfg_default.rq_size);
	...
	int ret = rpma_peer_create_srq(prestate->peer, Create_srq_cfg_default.cfg,
			&ibv_srq, &rcq);
...
static const struct CMUnitTest tests_create_srq[] = {
	...
	cmocka_unit_test_prestate_setup_teardown(
		...
		&prestate_OdpCapable),
	cmocka_unit_test_prestate_setup_teardown(
		create_srq__rpma_cq_new_ERRNO, setup__peer, teardown__peer,
		&prestate_OdpCapable),
	cmocka_unit_test_prestate_setup_teardown(
		create_srq__rpma_cq_new_ERRNO_subsequent_ERRNO2, setup__peer, teardown__peer,
		&prestate_OdpCapable),
	cmocka_unit_test_prestate_setup_teardown(
		create_srq__success, setup__peer, teardown__peer,
		&prestate_OdpCapable),
