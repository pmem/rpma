...
/*
 * create_qp_use_srq__rdma_create_qp_EAGAIN -- rdma_create_qp() fails with EAGAIN
 */
static void
create_qp_use_srq__rdma_create_qp_EAGAIN(void **peer_ptr)
{
	...
	expect_value(rdma_create_srq, attr->attr.max_wr, MOCK_SRQ_WR_SIZE_CUSTOM);
	expect_value(rdma_create_srq, attr->attr.max_sge, MOCK_SRQ_SGE_SIZE_CUSTOM);
	expect_value(rdma_create_srq, attr->attr.srq_limit, MOCK_SRQ_LIMIT_CUSTOM);
...
main(int argc, char *argv[])
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test_prestate_setup_teardown(create_qp__success,
				setup__peer, teardown__peer, &OdpCapable),
		cmocka_unit_test_prestate_setup_teardown(create_qp_use_srq__success,
				setup__peer, teardown__peer, &OdpCapable),
	};
