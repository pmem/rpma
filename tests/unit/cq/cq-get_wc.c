 ...
 * with each possible value of opcode and get a completion
 */
static void
...
/*
 * get_wc__success_all_opcode - handle ibv_poll_cq() successfully
 * with all possible values of opcode
 */
static void
get_wc__success_all_opcode(void **cq_ptr)
{
	struct rpma_cq *cq = *cq_ptr;
...
static const struct CMUnitTest tests_get_wc[] = {
	...
	cmocka_unit_test_setup_teardown(get_wc__success_each_opcode,
		setup__cq_new, teardown__cq_delete),
	cmocka_unit_test_setup_teardown(get_wc__success_all_opcode,
		setup__cq_new, teardown__cq_delete),
