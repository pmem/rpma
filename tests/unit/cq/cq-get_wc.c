...
get_wc__cq_NULL(void **unused)
{
	/* run test */
	struct ibv_wc wc = {0};
	int ret = rpma_cq_get_wc(NULL, &wc);
...
get_wc__wc_NULL(void **cq_ptr)
{
	...
	/* run test */
	int ret = rpma_cq_get_wc(cq, NULL);
...
/*
 * get_wc__poll_cq_2 - ibv_poll_cq() returns 2 which is
 * an abnormal situation
 */
static void
get_wc__poll_cq_2(void **cq_ptr)
{
	struct rpma_cq *cq = *cq_ptr;
...
static const struct CMUnitTest tests_get_wc[] = {
	...
	cmocka_unit_test_setup_teardown(get_wc__poll_cq_0,
		setup__cq_new, teardown__cq_delete),
	cmocka_unit_test_setup_teardown(get_wc__poll_cq_2,
		setup__cq_new, teardown__cq_delete),
