...
#define MOCK_GET_RCQ(cstate) \
	((cstate)->get_args.srq ? NULL : ((cstate)->get_args.rcq_size ? \
			MOCK_RPMA_RCQ : NULL))
...
#define CONN_REQ_NEW_TEST_WITH_AND_WITHOUT_RCQ(test_func) \
	CONN_REQ_NEW_TEST_SETUP_TEARDOWN_WITH_AND_WITHOUT_RCQ(test_func, \
		NULL, NULL)
#define CONN_REQ_NEW_TEST_SETUP_TEARDOWN_WITH_SRQ(test_func, \
		setup_func, teardown_func) \
	{#test_func "__with_srq", (test_func), (setup_func), \
		(teardown_func), &Conn_req_new_conn_cfg_custom_with_srq}
#define CONN_REQ_NEW_TEST_WITH_SRQ(test_func) \
	CONN_REQ_NEW_TEST_SETUP_TEARDOWN_WITH_SRQ(test_func, NULL, NULL)
...
#define CONN_REQ_TEST_WITH_AND_WITHOUT_RCQ(test_func) \
	CONN_REQ_TEST_SETUP_TEARDOWN_WITH_AND_WITHOUT_RCQ(test_func, \
		NULL, NULL)
#define CONN_REQ_TEST_SETUP_TEARDOWN_WITH_SRQ(test_func, \
		setup_func, teardown_func) \
	{#test_func "__with_srq", (test_func), (setup_func), \
		(teardown_func), &Conn_req_conn_cfg_custom_with_srq}
#define CONN_REQ_TEST_WITH_SRQ(test_func) \
	CONN_REQ_TEST_SETUP_TEARDOWN_WITH_SRQ(test_func, NULL, NULL)
...
extern struct conn_req_test_state Conn_req_conn_cfg_custom_with_srq;
...
extern struct conn_req_new_test_state Conn_req_new_conn_cfg_custom_with_srq;
