...
#define MOCK_GET_RCQ_DEL(cstate) \
	((!(cstate)->get_args.srq_rcq && (cstate)->get_args.rcq_size) ? \
			MOCK_RPMA_RCQ : NULL)
...
#define CONN_REQ_NEW_TEST_SETUP_TEARDOWN_WITH_AND_WITHOUT_SRQ_RCQ(test_func, \
	...
	{#test_func "__with_srq_rcq", (test_func), (setup_func), \
		(teardown_func), &Conn_req_new_conn_cfg_default_with_srq_rcq}
...
#define CONN_REQ_TEST_SETUP_TEARDOWN_WITH_AND_WITHOUT_SRQ_RCQ(test_func, \
	...
	{#test_func "__with_rcq", (test_func), (setup_func), \
		(teardown_func), &Conn_req_conn_cfg_default_with_srq_rcq}
