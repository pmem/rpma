...
/*
 * from_cm_event__create_comp_channel_EAGAIN -- rpma_cq_create()
 * fails with EAGAIN
 */
static void
from_cm_event__cq_create_EAGAIN(void **unused)
{
	/* configure mocks */
	struct rdma_cm_event event = CM_EVENT_CONNECTION_REQUEST_INIT;
	...
	expect_value(rpma_cq_create, cqe, MOCK_CQ_SIZE_DEFAULT);
	will_return(rpma_cq_create, NULL);
	will_return(rpma_cq_create, RPMA_E_PROVIDER);
	will_return(rpma_cq_create, EAGAIN);
...
from_cm_event__peer_create_qp_E_PROVIDER_EAGAIN(void **unused)
{
	...
	expect_value(rpma_cq_create, cqe, MOCK_CQ_SIZE_DEFAULT);
	will_return(rpma_cq_create, MOCK_RPMA_CQ);
 ...
 * fails with RPMA_E_PROVIDER+EAGAIN followed by rpma_cq_destroy() fails
 * with EIO
 */
static void
 ...
 * ENOMEM followed by rpma_cq_destroy() fails with EAGAIN
 */
static void
from_cm_event__malloc_ENOMEM_subsequent_EAGAIN(void **unused)
{
	...
	expect_value(rdma_destroy_qp, id, &id);
	will_return(rpma_cq_destroy, RPMA_E_PROVIDER);
	will_return(rpma_cq_destroy, EAGAIN); /* second error */
...
static const struct CMUnitTest test_from_cm_event[] = {
	...
	cmocka_unit_test(from_cm_event__cq_create_EAGAIN),
	cmocka_unit_test(
