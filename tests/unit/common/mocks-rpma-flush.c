...
/*
 * rpma_flush_mock_request -- rpma_flush_apm_request() mock
 */
int
rpma_flush_mock_request(struct ibv_qp *qp, struct rpma_flush *flush,
...
rpma_flush_new(struct rpma_peer *peer, struct rpma_flush **flush_ptr)
{
	...
	Rpma_flush.func = rpma_flush_mock_request;
