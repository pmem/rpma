...
/*
 * example__seq_init -- a sequential step of initialization; here should go
 * everything that is not thread-safe and/or should be prepared in a sequential
 * manner
 */
static void
example__seq_init(unsigned id, void *prestate, void **state_ptr,
		struct mtt_thread_result *tr)
{
	struct prestate *pr = (struct prestate *)prestate;
	struct state *st = (struct state *)calloc(1, sizeof(struct state));
	if (!st) {
		MTT_THREAD_ERR(tr, "calloc", errno);
		return;
	}
...
/*
 * example__init -- a parallel step of initialization; here should go
 * everything that is thread-safe and makes preparation for the test execution
 */
void
example__init(unsigned id, void *prestate, void **state_ptr,
		struct mtt_thread_result *tr)
{
...
/*
 * example__thread -- a test itself (parallel)
 */
static void
example__thread(unsigned id, void *prestate, void *state,
		struct mtt_thread_result *tr)
{
...
/*
 * example__fini -- a parallel step of cleanup; here should go everything that
 * can be cleaned up after the test in a thread-safe manner
 */
static void
example__fini(unsigned id, void *prestate, void **state_ptr,
		struct mtt_thread_result *tr)
{
	struct state *st = (struct state *)*state_ptr;
	st->output = *st->temp;
...
/*
 * example__seq_fini -- a sequential step of cleanup; here should go everything
 * that has to be cleaned up after the test but is not thread-safe
 */
static void
example__seq_fini(unsigned id, void *prestate, void **state_ptr,
		struct mtt_thread_result *tr)
{
	struct state *st = (struct state *)*state_ptr;
	printf("[%u] = %" PRIu64 "\n", id, st->output);
