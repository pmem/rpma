...
int
server_maina(char *addr, unsigned port, struct rpma_conn_private_data *pdata_exp)
{
	...
	pdata_exp->ptr = (void *)data;
	pdata_exp->len = sizeof(data);

	/*
	 * Wait for an incoming connection request, accept it and wait for its
	 * establishment.
	 */
	ret = server_accept_connection(ep, pdata_exp, &conn);
	if (ret)
