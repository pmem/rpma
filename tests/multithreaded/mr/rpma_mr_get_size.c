...
/*
 * thread -- gets memory region size
 */
static void
		...
		struct mtt_result *result)
{
	...
	int ret = rpma_mr_get_size(pr->mr, &st->mr_size);
	if (ret)
		MTT_RPMA_ERR(result, "rpma_mr_get_size", ret);

	if (st->mr_size != pr->mr_size_exp)
		MTT_ERR(result, "rpma_mr_get_size returned an unexpected value",
			EINVAL);
}
