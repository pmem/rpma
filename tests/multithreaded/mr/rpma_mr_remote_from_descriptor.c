		...
		struct mtt_result *result)
{
	...
	if ((ret = rpma_mr_remote_get_size(mr_ptr, &mr_size))) {
		MTT_RPMA_ERR(result, "rpma_mr_remote_get_size", ret);
	};

	if ((ret = rpma_mr_remote_get_size(*pr->mr_ptr, &pr_mr_size))) {
		MTT_RPMA_ERR(result, "rpma_mr_remote_get_size", ret);
	};

	/* verify the length of the received mr_remote */
	if (mr_size != pr_mr_size) {
		MTT_ERR_MSG(result, "Wrong length of the mr_remote", -1);
		return;
	}

	if (memcmp(mr_ptr, pr->mr_ptr, mr_size) != 0)
...
server_func(void *prestate)
{
	...
	return server_main(pst->addr, pst->port, &pst->pdata_exp);
}
