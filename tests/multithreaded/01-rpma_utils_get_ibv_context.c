...
/*
 * multi-threaded test for rpma_utils_get_ibv_context()
 */
...
static char *api_name = "rpma_utils_get_ibv_context";
...
static void *thread_main(void *arg)
{
	...
	if (ret) {
		...
		p_thread_args->thread_num, api_name, rpma_err_2str(ret));
		exit(-1);
	}
...
main(int argc, char *argv[])
{
	...
	if (p_threads == NULL || p_args == NULL) {
		fprintf(stderr, "%s failed: %s\n",
			api_name, rpma_err_2str(RPMA_E_NOMEM));
		exit(-1);
	}
	...
	for (i = 0; i < tn; i++) {
		...
		if ((ret = pthread_create(&p_threads[i], NULL, thread_main,
				&p_args[i])) != 0) {
			fprintf(stderr, "Can not start the thread #%d: %s\n",
				i, strerror(ret));
			return ret;
		}
	}
