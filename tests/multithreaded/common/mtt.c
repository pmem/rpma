...
/*
 * mtt_start_server -- start the server function in the background
 *                           (as a forked child process)
 */
static int
mtt_start_server(mtt_server_func server_func, void *prestate)
{
	int pid;
	int ret;
...
mtt_run(struct mtt_test *test, unsigned threads_num)
{
	...
	if (test->server_func) {
		pid_server = mtt_start_server(test->server_func,
