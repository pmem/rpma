...
struct mtt_thread_args {
	unsigned tid; /* a thread id */
	struct mtt_test *test;
	void *thread_state; /* a thread-specific state */
...
mtt_thread_main(void *arg)
{
	...
	if (test->thread_init_func) {
		test->thread_init_func(test->prestate, &ta->thread_state,
				ta->tid, tr);
		if (tr->ret) {
	...
	ta->test->thread_func(test->prestate, ta->thread_state, tr);

	if (test->thread_fini_func)
		test->thread_fini_func(&ta->thread_state, tr);
...
/*
 * mtt_broadcast_threads -- wait for threads to spawn and unblock all threads
 * synchronously
 */
static int
mtt_broadcast_threads(unsigned threads_num)
{
	int ret;
	int done = 0;
	...
	do {
		...
		if (mtt_sync.threads_num_waiting == threads_num) {
			...
			if (ret) {
				...
				/*
				 * Waiting for threads to join in this case
				 * may end up with a hang.
				 */
				exit(ret);
			}
...
/*
 * mtt_global_init -- initialize the global state
 */
static int
mtt_global_init()
{
	...
	if ((ret = pthread_mutex_init(&mtt_sync.mtx, NULL))) {
		MTT_ERR("pthread_mutex_init() failed: %s",
							strerror(ret));
		return ret;
	}
	if ((ret = pthread_cond_init(&mtt_sync.cond, NULL))) {
		MTT_ERR("pthread_cond_init() failed: %s",
							strerror(ret));
	...
	if ((ret = clock_gettime(CLOCK_REALTIME, &mtt_sync.timeout))) {
		MTT_ERR("clock_gettime() failed: %s",
				strerror(errno));
...
/*
 * mtt_global_fini -- clean up the global state
 */
static int
mtt_global_fini()
{
...
mtt_run(struct mtt_test *test, unsigned threads_num)
{
	...
	for (i = 0; i < threads_num; i++) {
		...
		if (test->thread_seq_init_func) {
			result = test->thread_seq_init_func(test->prestate,
				&ta->thread_state, i);
			if (result)
	...
	mtt_broadcast_threads(threads_num);
