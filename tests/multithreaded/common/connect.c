...
/*
 * connect.c -- common connection code of multithreaded tests
 */
...
/*
 * server_listen -- start a listening endpoint at addr:port
 */
int
server_listen(char *addr, unsigned port, struct rpma_peer **peer_ptr,
...
/*
 * server_wait_for_conn_close_and_disconnect -- wait for RPMA_CONN_CLOSED,
 * disconnect and delete the connection structure
 */
void
server_wait_for_conn_close_and_disconnect(struct rpma_conn **conn_ptr)
{
	enum rpma_conn_event conn_event = RPMA_CONN_UNDEFINED;
	int ret = 0;
...
/*
 * server_shutdown -- shutdown the endpoint and delete the peer object
 */
void
server_shutdown(struct rpma_peer **peer_ptr, struct rpma_ep **ep_ptr)
{
...
/*
 * client_connect -- connect with the server and get the private data
 */
int
client_connect(struct mtt_result *tr, char *addr, unsigned port,
...
/*
 * client_err_disconnect -- force disconnect and delete the peer object
 *                          in case of an error
 */
void
client_err_disconnect(struct rpma_conn **conn_ptr, struct rpma_peer **peer_ptr)
{
	(void) rpma_conn_disconnect(*conn_ptr);
	(void) rpma_conn_delete(conn_ptr);
	(void) rpma_peer_delete(peer_ptr);
...
/*
 * client_disconnect -- disconnect and delete the peer object
 */
void
client_disconnect(struct mtt_result *tr, struct rpma_conn **conn_ptr,
