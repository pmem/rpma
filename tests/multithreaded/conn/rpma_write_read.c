...
thread_seq_init(unsigned id, void *prestate, void **state_ptr, struct mtt_result *tr)
{
	...
	*state_ptr = ts;
	...
	ts->local_mr_size = MAX_STR_LEN;
	ts->local_ptr = mtt_malloc_aligned(ts->local_mr_size, tr);
	...
	/* register the memory */
	ret = rpma_mr_reg(pr->peer, ts->local_ptr, ts->local_mr_size,
...
/*
 * thread -- write and verify the data using rpma_read
 */
static void
thread(unsigned id, void *prestate, void *state, struct mtt_result *result)
{
	...
	/* fill the source memory with a content */
	memcpy(ts->local_ptr, HELLO_STR, ts->local_mr_size);
	...
	memset(ts->local_ptr, 0, ts->local_mr_size);

	/* post an RDMA read operation */
	ret = rpma_read(ts->conn, ts->mr_local_ptr, 0, ts->mr_remote_ptr, 0, ts->mr_remote_size,
	...
	if (memcmp(ts->local_ptr, HELLO_STR, sizeof(HELLO_STR)) != 0)
