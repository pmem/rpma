...
thread_seq_init(unsigned id, void *prestate, void **state_ptr, struct mtt_result *tr)
{
	...
	*state_ptr = ts;
	...
	size_t local_mr_size = MAX_STR_LEN;
	ts->local_ptr = mtt_malloc_aligned(local_mr_size, tr);
	...
	/* register the memory */
	ret = rpma_mr_reg(pr->peer, ts->local_ptr, local_mr_size, RPMA_MR_USAGE_READ_DST,
				&ts->mr_local_ptr);
	if (ret) {
