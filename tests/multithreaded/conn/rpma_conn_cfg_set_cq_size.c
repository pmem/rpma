...
thread(unsigned id, void *prestate, void *state, struct mtt_result *tr)
{
	...
	if ((ret = rpma_conn_cfg_set_cq_size(st->cfg_ptr, Q_SIZE_EXP))) {
		MTT_RPMA_ERR(tr, "rpma_conn_cfg_set_cq_size", ret);
		return;
	}

	if ((ret = rpma_conn_cfg_get_cq_size(st->cfg_ptr, &cq_size))) {
		MTT_RPMA_ERR(tr, "rpma_conn_cfg_get_cq_size", ret);
		return;
	}

	if (cq_size != Q_SIZE_EXP)
		MTT_ERR(tr, "cq_size != Q_SIZE_EXP", EINVAL);
}
