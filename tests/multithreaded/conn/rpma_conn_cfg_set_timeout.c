...
#define MTT_CONN_CFG_TIMEOUT_MS 2000
...
/*
 * init -- allocate state and creates a new connection configuration object
 */
void
...
/*
 * thread -- set connection establishment timeout
 * and check if its value is expected
 */
static void
thread(unsigned id, void *prestate, void *state, struct mtt_result *tr)
{
	...
	if ((ret = rpma_conn_cfg_set_timeout(st->cfg_ptr,
				MTT_CONN_CFG_TIMEOUT_MS))) {
		MTT_RPMA_ERR(tr, "rpma_conn_cfg_set_timeout", ret);
		return;
	}

	if ((ret = rpma_conn_cfg_get_timeout(st->cfg_ptr,
				&timeout_ms))) {
		MTT_RPMA_ERR(tr, "rpma_conn_cfg_set_timeout", ret);
		return;
	}

	if (timeout_ms != MTT_CONN_CFG_TIMEOUT_MS)
		MTT_ERR(tr, "rpma_conn_cfg_get_timeout got an unexpected value",
				EINVAL);
}
