...
/*
 * server_rpma_read.c -- a server of the rpma_read MT test
 */
...
server_init(struct server_res *svr, struct rpma_peer *peer)
{
	...
	/* allocate a memory */
	size_t mr_local_size = strlen(HELLO_STR) + 1;
	...
	memcpy(svr->local_ptr, HELLO_STR, mr_local_size);

	/* register the memory */
	ret = rpma_mr_reg(peer, svr->local_ptr, mr_local_size, RPMA_MR_USAGE_READ_SRC,
				&svr->mr_local_ptr);
	if (ret)
		goto err_free;
