 ...
 */
static int
get_ibv_context__thread_init(struct mt_test_state *args, void *thread_args)
{
 ...
 */
static void *
get_ibv_context__thread(void *arg)
{
	...
	ret = rpma_utils_get_ibv_context(ta->addr, RPMA_UTIL_IBV_CONTEXT_LOCAL,
			&dev);
	if (ret)
		MT_TEST_THREAD_RPMA_ERR(ta, "rpma_utils_get_ibv_context", ret);
...
main(int argc, char *argv[])
{
	...
	return mt_test_run(argc, argv, &test);
}
