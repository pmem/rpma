...
/*
 * rpma_peer_cfg_new.c -- create a new peer configuration object
 */
...
main(int argc, char *argv[])
{
	...
	pthread_t *p_threads;
	p_threads = calloc(thread_num, sizeof(pthread_t));
	if (p_threads == NULL) {
		fprintf(stderr, "malloc() failed");
		return -1;
	}

	struct thread_args *threads_args = calloc(thread_num,
