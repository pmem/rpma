...
/*
 * test_addr_NULL - test NULL address parameter
 */
static void
test_addr_NULL(void **state)
{
	(void) state; /* unused */
	...
	assert_int_equal(errno, MOCK_ERRNO_1);
	assert_null(dev);
}
 ...
 */
static void
test_getaddrinfo_failed(void **state)
{
	(void) state; /* unused */
	...
	/* run test */
	struct ibv_context *dev = NULL;
	int ret = rpma_utils_get_ibv_context("", &dev);
 ...
 */
static void
test_create_id_failed(void **state)
{
	(void) state; /* unused */
 ...
 */
static void
test_bind_addr_failed(void **state)
{
	(void) state; /* unused */
	...
	/* verify the results */
	assert_int_equal(ret, RPMA_E_PROVIDER);
	assert_int_equal(errno, MOCK_ERRNO_2);
	assert_null(dev);
}
