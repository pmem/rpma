...
static const char Buff_exp[] = BUFF_EXP;
...
setup__mr_remote(void **mr_ptr)
{
	...
	int ret = rpma_mr_deserialize(Buff_exp, SERIALIZED_SIZE_EXP, &mr);
...
{
	/* run test */
	int ret = rpma_mr_deserialize(Buff_exp, SERIALIZED_SIZE_EXP, NULL);
...
test_deserialize__too_big_size(void **unused)
{
	/* run test */
	struct rpma_mr_remote *mr = NULL;
	int ret = rpma_mr_deserialize(Buff_exp, SERIALIZED_SIZE_EXP + 1, &mr);
