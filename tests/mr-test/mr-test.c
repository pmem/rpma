...
#define MOCK_USAGE_MAX	MOCK_USAGE_3 /* the maximum value of usage */
...
setup__reg_success(void **state)
{
	...
	/*
	 * Set value of usage.
	 * First check if usage was passed thru value:
	 */
	temp = (int)(uintptr_t)(*state);
	if (temp < 0 || temp > MOCK_USAGE_MAX) {
		/* ... then check if usage was passed thru pointer: */
		temp = *(int *)(*state);
		if (temp < 0 || temp > MOCK_USAGE_MAX)
			return -1; /* passed wrong value of usage */
	}
	usage = temp;
 ...
 */
static void
test_dereg__failed_E_PROVIDER(void **state)
{
	int usage = MOCK_USAGE_3;
	*state = &usage;
	 ...
	 */
	int ret = setup__reg_success(state);
	assert_int_equal(ret, MOCK_OK);
	struct rpma_mr_local *mr = *state;
...
main(int argc, char *argv[])
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test_prestate_setup_teardown(
			...
			(void *)(uintptr_t)MOCK_USAGE_0),
		cmocka_unit_test_prestate_setup_teardown(
			test_reg_dereg__success,
			setup__reg_success, teardown__dereg_success,
			(void *)(uintptr_t)MOCK_USAGE_1),
		cmocka_unit_test_prestate_setup_teardown(
			test_reg_dereg__success,
			setup__reg_success, teardown__dereg_success,
			(void *)(uintptr_t)MOCK_USAGE_2),
		cmocka_unit_test_prestate_setup_teardown(
			test_reg_dereg__success,
			setup__reg_success, teardown__dereg_success,
			(void *)(uintptr_t)MOCK_USAGE_3),
