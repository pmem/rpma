...
connect_test_conn_ptr_NULL(void **cstate_ptr)
{
	...
	/* run test */
	int ret = rpma_conn_req_connect(&cstate->req, NULL, 0, NULL);
 ...
 */
static void
connect_test_pdata_NULL_pdata_len_not_0(void **cstate_ptr)
{
	struct conn_req_test_state *cstate = *cstate_ptr;

	/* run test */
	struct rpma_conn *conn = NULL;
	int ret = rpma_conn_req_connect(&cstate->req, NULL, 1, &conn);
...
connect_test_accept_EAGAIN(void **unused)
{
	...
	/* run test */
	struct rpma_conn *conn = NULL;
	int ret = rpma_conn_req_connect(&cstate->req, NULL, 0, &conn);
...
/*
 * connect_test_success -- rpma_conn_req_connect() success
 */
static void
connect_test_success(void **unused)
{
	/* WA for cmocka/issues#47 */
	struct conn_req_test_state *cstate = NULL;
	assert_int_equal(conn_req_setup((void **)&cstate), 0);
...
main(int argc, char *argv[])
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test_setup_teardown(
			connect_test_pdata_NULL_pdata_len_not_0,
			conn_req_setup, conn_req_teardown),
		...
		cmocka_unit_test(connect_test_success),
	};
