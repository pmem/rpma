...
main(int argc, char *argv[])
{
	...
	memcpy(used.buf, (char *)mr_ptr, sizeof(used));
	printf("used value: %ld\n", used.uint64_t);

	if (remote_size <= used.uint64_t) {
		fprintf(stderr,
				"Log size exhausted.\n");
		goto err_mr_remote_delete;
	}
	...
	for (int i = 3; i < argc; ++i) {
		...
		if ((ret = rpma_write(conn, remote_mr, used.uint64_t,
				local_mr, 0, word_size,
				RPMA_F_COMPLETION_ON_ERROR, NULL)))
			break;

		if ((ret = rpma_flush(conn, remote_mr, used.uint64_t,
				word_size, flush_type,
				RPMA_F_COMPLETION_ON_ERROR, NULL)))
			break;

		used.uint64_t += (uint64_t)word_size;
