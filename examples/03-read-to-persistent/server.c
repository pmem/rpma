...
main(int argc, char *argv[])
{
	...
	struct pmem2_map *dst_map;
	int fd;

	if (argc >= 4) {
		/* map pmem */
		char *path = argv[3];
		ret = common_pmem2_map(path, &fd, &dst_map, &dst_offset,
				&dst_initialized);
		if (ret)
			return -1;
		...
		persist_fn = pmem2_get_persist_fn(dst_map);
		dst_ptr = pmem2_map_get_address(dst_map);
		dst_size = pmem2_map_get_size(dst_map);
	...
	if (dst_plt == RPMA_MR_PLT_PERSISTENT) {
		persist_fn((char *)dst_ptr + dst_offset, src_data->data_len);
		common_write_signature(dst_map);
	}
#endif
