...
main(int argc, char *argv[])
{
	...
	if (argc >= 4) {
		 ...
		 */
		dst_has_content = (strncmp(dst_ptr, SIGNATURE_STR,
					SIGNATURE_LEN) == 0);
	...
	if (dst_plt == RPMA_MR_PLT_PERSISTENT) {
		...
		/*
		 * If the destination was empty (has no content) before reading
		 * content the signature has to be written to mark the new
		 * content as valid.
		 */
		if (!dst_has_content) {
