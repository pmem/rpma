...
# valgrind test with suppression passed under rdma-core v33.0 debug libraries.
if [ x"$VLD_TEST" == x"--valgrind" ]; then
	...
	echo "Running examples with Valgrind is tuned for rdma-core v33.0 debug build (EXPERIMENTAL)"
	echo "It may fail for any other version and for the release build."
	echo
fi
