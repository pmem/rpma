...
MODULE="rdma_rxe"
SCRIPT="utils/rxe_config.sh"

function verify_SoftRoCE() {
	if [ $(lsmod | grep -e $MODULE | wc -l) -lt 1 ]; then
		echo "Error: the '$MODULE' module is not loaded, run the '$SCRIPT' script."
		exit 1
	fi

	if ! which rdma > /dev/null; then
		echo "Error: cannot find the 'rdma' command. Install the 'iproute2' package."
		exit 1
	fi
	...
	if [ $RDMA_LINKS -eq 0 ]; then
		echo "SoftRoCE has not been configured yet, run the '$SCRIPT' script."
		exit 1
	fi
