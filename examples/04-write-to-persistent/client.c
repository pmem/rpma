...
main(int argc, char *argv[])
{
	...
	struct rpma_mr_remote *write_dst_mr = NULL;
	size_t dst_size = 0;
	size_t dst_offset = 0;
	struct rpma_mr_local *src_mr = NULL;
	struct rpma_completion cmpl;

	struct rpma_mr_local *read_src_mr = NULL;
	...
	/* alloc memory for RPMA read */
	void *buff = malloc_aligned(8);
	if (buff == NULL)
		return -1;
	...
	/* register the memory RDMA read */
	ret = rpma_mr_reg(peer, buff, 8,
	...
	ret = rpma_mr_remote_from_descriptor(desc, &write_dst_mr);
	if (ret) {
	...
	ret = rpma_mr_remote_get_size(write_dst_mr, &dst_size);
	if (ret) {
	...
	ret = rpma_write(conn, write_dst_mr, dst_offset, src_mr,
			mr_size, KILOBYTE,
			RPMA_F_COMPLETION_ALWAYS, NULL);
	if (ret)
		goto err_conn_disconnect;

	ret = rpma_read(conn, read_src_mr, 0, write_dst_mr,
			dst_offset, 8,
			RPMA_F_COMPLETION_ALWAYS, NULL);
	if (ret)
		goto err_conn_disconnect;
...
err_mr_remote_delete:
	...
	ret = rpma_mr_remote_delete(&write_dst_mr);
	if (ret)
