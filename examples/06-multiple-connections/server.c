...
/*
 * client_init -- find a slot for the incoming client
 */
struct client_res *
client_init(struct server_res *svr, struct rpma_conn_req *req)
{
...
void conn_cmpl(struct custom_event *ce);
void conn_event(struct custom_event *ce);
...
client_add_to_epoll(struct client_res *clnt, int epoll)
{
	...
	ret = epoll_add(epoll, fd, clnt, conn_event, &clnt->ev_conn_event);
	if (ret)
		return ret;
	...
	ret = epoll_add(epoll, fd, clnt, conn_cmpl, &clnt->ev_conn_cmpl);
	if (ret)
		epoll_delete(epoll, &clnt->ev_conn_event);
...
/*
 * client_fini -- release client's resources
 */
void
client_fini(struct client_res *clnt)
{
...
ep_incoming_func(struct custom_event *ce)
{
	...
	struct client_res *clnt = NULL;
	if ((clnt = client_init(svr, req)) == NULL) {
...
main(int argc, char *argv[])
{
	...
	ret = epoll_add(svr.epoll, ep_fd, &svr, ep_incoming_func,
			&svr.ev_incoming);
	if (ret)
		goto err_ep_shutdown;
