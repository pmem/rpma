...
int
common_pmem_signature_check(char *path, size_t size, struct common_mem *mem)
{
	...
	if (strncmp(mem->mr_ptr, SIGNATURE_STR, SIGNATURE_LEN) != 0) {
		if (size == 0) {
			...
			write_hello_str((struct hello_t *)((uintptr_t)mem->mr_ptr
				+ mem->data_offset), en);
			mem->persist((struct hello_t *)((uintptr_t)mem->mr_ptr + mem->data_offset),
