...
main(int argc, char *argv[])
{
	...
	if (ret) {
		...
		print_error("The server has not provided a remote memory region. (the connection's private data is empty)",
				ret);
		goto err_conn_disconnect;
	}
	...
	if (ret) {
		...
		fprintf(stderr, "Remote memory region size too big to reading to the sink buffer of the assumed size (%zu > %d)\n",
				src_size, KILOBYTE);
		goto err_mr_remote_delete;
	} else {
	...
	if (ret) {
		...
		fprintf(stderr, "rpma_conn_next_completion returned a completion of an unexpected operation: %d\n",
				cmpl.op);
	} else if (cmpl.op_status != IBV_WC_SUCCESS) {
		fprintf(stderr, "rpma_conn_next_completion returned an unexpected operation status: %d\n",
				cmpl.op_status);
	} else {
