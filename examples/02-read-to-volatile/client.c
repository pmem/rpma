...
main(int argc, char *argv[])
{
	...
	/*
	 * use rpma_mr_get_ptr() and rpma_mr_get_size() to verify if
	 * the raw memory address and size are registered successfully
	 */
	size_t get_size = 0;
	void *get_dst_ptr = NULL;

	ret = rpma_mr_get_ptr(dst_mr, &get_dst_ptr);
