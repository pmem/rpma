...
function lat_appendix()
{
    ...
    $TOOLS_PATH/csv_compare.py \
        --output_title "Appendix $no. Complete statistics of $title collected from all of the platforms used" \
...
function lat_figures()
{
    ...
    title_appendices=( \
        'latencies' \
        '99.0% and 99.9% latency percentiles' \
        '99.99% and 99.999% latency percentiles')
    for index in "${!layouts[@]}"; do
        layout="${layouts[$index]}"
        title_appendix="${title_appendices[$index]}"
        $TOOLS_PATH/csv_compare.py \
            --output_title "Figure $figno. $title $title_appendix" \
...
if [ -z "$READ_BW_MACHINE" ]; then
    ...
    bw_figure \
        ...
        'Figure 5. rpma_read() vs ib_read_bw bandwidth against block size' \
        'bs' \
...
if [ -z "$READ_BW_MACHINE" ]; then
    ...
    bw_figure \
        ...
        'Figure 6. rpma_read() vs ib_read_bw bandwidth against the number of threads' \
        'threads' \
...
if [ -z "$WRITE_BW_MACHINE" ]; then
    ...
    bw_figure \
        ...
        'Figure 8. rpma_write() + rpma_flush() to DRAM vs to PMEM bandwidth against block size' \
        'bs' \
...
if [ -z "$WRITE_BW_MACHINE" ]; then
    ...
    bw_figure \
        ...
        'Figure 9. rpma_write() + rpma_flush() to DRAM vs to PMEM bandwidth against number of threads' \
        'threads' \
