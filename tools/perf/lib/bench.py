...
class Bench:
    ...
    def __init__(self, config: dict, parts: list,
                 ...
                 result_dir: str):
        """It is the core of the whole benchmarking framework.

        To setup the benchmarking it needs:

        - `Bench.config` the configuration of the benchmarking system
        - `Bench.parts` the list of the names of all parts covered. To cover
          a part the `Bench` object has to build all figures contained within
          the part.
        ...
        - `Bench.requirements` the dict of the requirements for all the series
          covered. Each of the requirements contains a list of the series
          making the same requirement. For the efficiency purposes, the list of
          series within the requirement does not contain duplicates so a single
          series may be later on used in more than one figure.
        ...
        used to execute the planned series. When it will return `True` all
    ...
    def figures(self) -> list:
        ...
        The 'flat' list is of all figures coming from all parts covered.
    ...
    def requirements(self) -> dict:
        ...
        Where keys are identifier of the requirements and the values are
    ...
    def new(cls, config: dict, parts: list, result_dir: str) -> 'Bench':
        ...
        Args:
            ...
            parts: a list of parts to be covered. Each of the parts is
              a dictionary. The `part['input_file']` is exptected to be the name
              of the source JSON file. The `part['json']` is exptect to be
        ...
        # generate a list of unique requirements to meet
    ...
    def carry_on(cls, cache: dict, skip_undone=False) -> 'Bench':
        ...
        Args:
            cache: a cache of the `Bench` object. The `bench['input_file']` is
              expected to be the name of the source JSON file.
              The `bench['json]` is expected to be a dictionary in the format
    ...
    def cache(self) -> None:
        ...
        - `cache['config']` which store the `Bench.config` property
        - `cache['parts']` which store the `Bench.parts` property
        - `cache['figures']` which store the `Bench.figures` property
        - `cache['requirements']` which store the `Bench.requirements` property
    ...
    def run(self) -> bool:
        """run all series one-by-one & collect all series for `Bench.figures`

        Browse `Bench.requirements` and if one of them is met
        (`lib.Requirement.Requirement.is_met()`) it executes all series forcing
        the given requirement one-by-one (for details please see
