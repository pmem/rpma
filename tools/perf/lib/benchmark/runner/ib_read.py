...
class IbReadRunner:
    ...
    def __set_common_vars(self, _settings):
        ...
        _settings['bs_opt'] = ['--size', str(_settings['bs'])]\
                              if _settings['bs_opt'] else []
        _settings['qp_opt'] = ['--qp', str(_settings['threads'])]\
                              if _settings['qp_opt'] else []
        _settings['dp_opt'] = [''.join(['--tx-depth=',\
                              str(_settings['iodepth'])])]\
    ...
    def __server_start(self, _settings):
        ...
        args = ['numactl', '-N', r_numa_n, r_ib_path,
                *_settings['bs_opt'], *_settings['qp_opt'],
                *_settings['dp_opt'], *r_aux_params]
        cmd = ' '.join(args)
