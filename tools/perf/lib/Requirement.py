...
class Requirement:
    ...
    def __pick(self, benchmarks: dict) -> None:
        """Filter out and collect all benchmarks making the requirement"""
        self.__benchmarks = {}
    ...
    def uniq(cls, benchmarks: dict) -> list:
        ...
        `lib.benchmark.base.Benchmark.requirements` taken out from elements
        of the `benchmarks` dictionary are used to create a list of
        `Requirement` objects. From this list are removed all duplicates.
        Having a set of unique `Requirement` objects,
        `lib.benchmark.base.Benchmark` objects from the `benchmarks` `dict`
        are filtered out and assigned to the `Requirement` object which they
        are making.
