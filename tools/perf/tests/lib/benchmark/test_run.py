...
REQ_DUMMY = {'direct_write_to_pmem': True}
...
def test_incomplete_benchmark(oneseries_dummy, key, tmpdir):
    ...
    with pytest.raises(ValueError):
        benchmark.run(CONFIG_DUMMY, REQ_DUMMY, str(tmpdir))
...
def test_no_server_ip(benchmark_dummy, tmpdir):
    ...
    with pytest.raises(ValueError):
        benchmark_dummy.run(config, REQ_DUMMY, str(tmpdir))
...
def test_dummy_runner(benchmark_dummy, tmpdir, monkeypatch):
    ...
    benchmark_dummy.run(CONFIG_DUMMY, REQ_DUMMY, str(tmpdir))
...
def test_bash_runner(benchmark_bash, tmpdir, monkeypatch):
    ...
    benchmark_bash.run(CONFIG_DEFAULT, REQ_DUMMY, str(tmpdir))
    assert run_mock_used
...
def test_executor_runner(benchmark_executor, tmpdir, monkeypatch):
    ...
    benchmark_executor.run(CONFIG_DEFAULT, REQ_DUMMY, str(tmpdir))
    assert run_mock_used
