...
def test_incomplete(key, monkeypatch):
...
def test_ib_read(mode, monkeypatch):
    ...
    def run_mock(args, env):
        ...
        return ProcessMock()
    monkeypatch.setattr(subprocess, 'run', run_mock)
    oneseries = {'tool': TOOL_IB_READ, 'mode': mode, 'id': ID_DUMMY}
    benchmark = Benchmark(oneseries)
    benchmark.run(CONFIG_DUMMY, RESULT_DIR)
...
def test_rpma_fio_bench(tool_mode, readwrite, mode, monkeypatch):
    ...
    def run_mock(args, env):
        ...
        return ProcessMock()
    monkeypatch.setattr(subprocess, 'run', run_mock)
    oneseries = {'tool': TOOL_RPMA_FIO_BENCH, 'mode': mode,
        'tool_mode': tool_mode, 'rw': readwrite, 'id': ID_DUMMY}
    benchmark = Benchmark(oneseries)
    benchmark.run(CONFIG_DUMMY, RESULT_DIR)
    assert benchmark.is_done()
