...
def run_mock_fail(_self):
    """failing mock of *Runner.run()"""
    func = re.match(r'\S+', str(_self)).group(0)[1:]
...
def test_ib_read_success(benchmark_ib_read, tmpdir, monkeypatch):
    ...
    monkeypatch.setattr(lib.benchmark.runner.ib_read.IbReadRunner, 'run',
                        run_mock_pass)
    lib.benchmark.runner.Executor.run(benchmark_ib_read, CONFIG_BIG,
                                      str(tmpdir))
...
def test_fio_success(benchmark_fio, tmpdir, monkeypatch):
    ...
    monkeypatch.setattr(lib.benchmark.runner.fio.FioRunner, 'run',
                        run_mock_pass)
    lib.benchmark.runner.Executor.run(benchmark_fio, CONFIG_BIG, str(tmpdir))
    # marking a benchmark as 'done' is done outside of the runner
...
def test_ib_read_missing_key(oneseries_ib_read, key, tmpdir, monkeypatch):
    ...
    with pytest.raises(ValueError):
        lib.benchmark.runner.Executor.run(benchmark, CONFIG_BIG, str(tmpdir))
    # marking a benchmark as 'done' is done outside of the runner
