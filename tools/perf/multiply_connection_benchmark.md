...
For every `*server.fio` file from the folder `rpma/tools/perf/fio_jobs` `serverip` from `[global]` section shall be moved to the `[server]` section and add `filename1` and `filename2`:
```
[global]
...
# serverip=${serverip} #comment out or removed from [global] section
...
filename=${filename1}
```
Additionally, we have to create a new separate `[server]` section with a explicitly set `server IP` and `filename`:
```sh
[server]
