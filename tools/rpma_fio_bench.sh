...
# all modes except "all"
MODES="bw-bs bw-dp-exp bw-dp-lin bw-th lat"

case $MODE in
all|bw-bs|bw-dp-exp|bw-dp-lin|bw-th|lat)
	;;
*)
	usage "Wrong mode: $MODE"
	;;
...
if [ "$OP" == "all" -a "$MODE" == "all" ]; then
	...
	for o in $OPS; do
		benchmark_one $o $MODE $SERVER_IP
	done
elif [ "$MODE" == "all" ]; then
	for m in $MODES; do
		benchmark_one $OP $m $SERVER_IP
	done
else
	benchmark_one $OP $MODE $SERVER_IP
fi
