...
function benchmark_one() {
	...
	if [ -z "$REMOTE_JOB_PATH" ]; then
		REMOTE_JOB_PATH=${DIR}/librpma${PERSIST_MODE}-server-${TIMESTAMP}.fio
	fi
	...
	for i in $(seq 0 $(expr $ITERATIONS - 1)); do
		...
		sshpass -p "$REMOTE_PASS" scp -o StrictHostKeyChecking=no \
			./fio_jobs/librpma${PERSIST_MODE}-server.fio \
		...
		serverip=$SERVER_IP blocksize=$BS numjobs=$TH iodepth=${DP} readwrite=${OP} \
			ramp_time=$RAMP_TIME runtime=$RUNTIME ioengine=librpma${PERSIST_MODE}_client \
			$TRACER ${FIO_PATH}fio \
