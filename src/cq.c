...
/*
 * rpma_cq_get_fd -- get a file descriptor of the completion event
 * channel associated with completion queue object
 */
int
...
/*
 * rpma_cq_wait -- wait for completions from completion queue object
 */
int
...
/*
 * rpma_cq_get -- receive an operation completion from completion queue object
 */
int
...
/*
 * rpma_cq_create -- create a completion queue object
 */
int
rpma_cq_create(struct ibv_context *verbs, int cqe, struct rpma_cq **cq_ptr)
{
	if (verbs == NULL || cq_ptr == NULL)
		return RPMA_E_INVAL;

	int ret = 0;

	/* create a completion channel */
	struct ibv_comp_channel *channel = ibv_create_comp_channel(verbs);
	if (channel == NULL) {
	...
	/* create a CQ */
	struct ibv_cq *cq = ibv_create_cq(verbs, cqe,
				NULL /* cq_context */,
				channel /* channel */,
				0 /* comp_vector */);
	if (cq == NULL) {
...
/*
 * rpma_cq_destroy -- destroy a completion queue object
 */
int
