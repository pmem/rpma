...
/** 3
 * rpma_write_atomic_with_imm - initiate the atomic write operation with
 * immediate data
 *
 * SYNOPSIS
 *
 ...
 *	struct rpma_mr_local;
 *	struct rpma_mr_remote;
 *	int rpma_write_atomic_with_imm(struct rpma_conn *conn,
 *			struct rpma_mr_remote *dst, size_t dst_offset,
 *			const struct rpma_mr_local *src,  size_t src_offset,
 *			int flags, uint32_t imm, const void *op_context);
 *
 * DESCRIPTION
 * rpma_write_atomic_with_imm() initiates the atomic write operation with
 * immediate data (transferring data from the local memory to the remote
 * memory). The atomic write operation allows transferring 8 bytes of data
 * (RPMA_ATOMIC_WRITE_ALIGNMENT) and storing them atomically in the remote
 * memory.
 * The attribute flags set the completion notification indicator:
 * - RPMA_F_COMPLETION_ON_ERROR - generate the completion on error
 * - RPMA_F_COMPLETION_ALWAYS - generate the completion regardless of result of
 ...
 *
 * RETURN VALUE
 * The rpma_write_atomic_with_imm() function returns 0 on success or
 * a negative error code on failure.
 *
 * ERRORS
 * rpma_write_atomic_with_imm() can fail with the following errors:
 *
 * - RPMA_E_INVAL - conn, dst or src is NULL
 * - RPMA_E_INVAL - dst_offset is not aligned to 8 bytes
 * - RPMA_E_INVAL - flags are not set
