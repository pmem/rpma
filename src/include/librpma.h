 ...
 *
 * DESCRIPTION
 * rpma_conn_req_get_private_data() obtains the pointer to the private data
 * given by the other side of the connection before connection is fully
 * established.
 * In rpma_conn_req_get_private_data() provides access to connection private
 * data before connection is established in opposite to
 * rpma_conn_get_private_data(3) which can be used after connection is fully
 * established. In this case connection private data could be used as an input
 * to server side configuration priory to connection accept.
 *
 * RETURN VALUE
 ...
 *
 * - RPMA_E_INVAL - req_ptr or pdata is NULL
 *
 * SEE ALSO
