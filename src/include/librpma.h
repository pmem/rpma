 ...
 *
 * DESCRIPTION
 * rpma_mr_get_descriptor() writes a network-transferable description of the
 * provided local memory region. Once the descriptor is transferred to the other
 * side it can be consumed by rpma_mr_remote_from_descriptor() to create a
 * remote memory region's structure which allows transferring data between the
 * peers.
 *
 * ERRORS
 ...
 * - RPMA_E_NOMEM - out of memory
 */
int rpma_mr_remote_from_descriptor(const struct rpma_mr_descriptor *desc,
		struct rpma_mr_remote **mr_ptr);
