...
/** 3
 * rpma_cq_get_wc - receive a completion of an operation
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 ...
 *
 * DESCRIPTION
 * rpma_cq_get_wc() receives the next available completion of an already
 * posted operation by a wc pointer. All operations generate completion
 * on error. The operations posted with the RPMA_F_COMPLETION_ALWAYS flag
 * also generate a completion on success. Please see ibv_poll_cq(3)'s manual
 * for details about struct ibv_wc.
 *
 * Note that if the provided cq is the main CQ and the receive CQ is present
 * on the same connection this function won't return RPMA_OP_RECV and
 * RPMA_OP_RECV_RDMA_WITH_IMM at any time. The receive CQ has to be used
 ...
 *
 * RETURN VALUE
 * The rpma_cq_get_wc() function returns 0 on success or a negative error
 * code on failure. On success, it writes the first available completion to wc.
 * If the status of wc is not equal to IBV_WC_SUCCESS then only the following
 * members of wc are valid: wr_id, status, qp_num, and vendor_err.
 *
 * ERRORS
 * rpma_cq_get_wc() can fail with the following errors:
 *
 * - RPMA_E_INVAL - cq or wc is NULL
 * - RPMA_E_NO_COMPLETION - no completions available
 * - RPMA_E_PROVIDER - ibv_poll_cq(3) failed with a provider error
 * - RPMA_E_UNKNOWN - ibv_poll_cq(3) failed but no provider error is available
 *
