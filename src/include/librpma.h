 ...
 * a userspace process via the file descriptor. The default mode of file
 * descriptor is blocking and it can be changed to the non-blocking mode
 * by fcntl(2). The CQ is either the connection's main CQ or the receive
 * CQ, please see rpma_conn_get_cq(3) and rpma_conn_get_rcq(3) for details.
 *
 * RETURN VALUE
