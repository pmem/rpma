...
/** 3
 * rpma_mr_get_mm_ptr - a local registered memory address
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 *
 *	struct rpma_mr_local;
 *	int rpma_mr_get_mm_ptr(const struct rpma_mr_local *mr, void **mm_ptr);
 *
 * DESCRIPTION
 * rpma_mr_get_mm_ptr() gets address of the local registered memory.
 *
 * RETURN VALUE
 * The rpma_mr_get_mm_ptr() function returns 0 on success or a negative error
 * code on failure. rpma_mr_get_mm_ptr() does not set *mm_ptr value on failure.
 *
 * ERRORS
 * rpma_mr_get_mm_ptr() can fail with the following error:
 *
...
/** 3
 * rpma_mr_get_size - a local registered memory size
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 ...
 *
 * DESCRIPTION
 * rpma_mr_get_size() gets size of the local registered memory.
 *
 * RETURN VALUE
