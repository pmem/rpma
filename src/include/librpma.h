 ...
 *			struct ibv_wc *wc, int *got_entries);
 *
 * DESCRIPTION
 ...
 * the got_entries argument if it is not NULL. It can be NULL only if
 ...
 * the wc and got_entries. If the status of a completion is not equal to
 * IBV_WC_SUCCESS then only the following attributes are valid: wr_id, status,
