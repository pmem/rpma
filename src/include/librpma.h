 ...
 * which allows remote peers to write to platform's PMem in a persistent way.
 * It may be impossible because of e.g. caching mechanisms existing on
 * the data way. Assuming **Direct Write to PMem** is possible when it is not
 * may corrupt data on your PMem so it is not enabled by default.
 *
 * On the current platforms, the only thing you have to do to enable **Direct**
 * **Write to PMem** is turning off Direct Data I/O (DDIO).
 * Sometimes, you can turn off DDIO either globally for the whole platform
 * or for a specific PCIe Root Port. For details, please see
