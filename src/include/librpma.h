 ...
 * THREAD SAFETY
 *
 * The analysis of thread safety of the librpma library is described in details
 * in the THREAD_SAFETY.md file:
 *
 ...
 * rpma_conn_cfg_set_compl_channel(3), rpma_conn_cfg_set_cq_size(3),
 ...
 *
 * SEE ALSO
 * rpma_conn_cfg_new(3), rpma_conn_wait(3), rpma_conn_cfg_set_compl_channel(3),
 * librpma(7) and https://pmem.io/rpma/
 */
int rpma_conn_cfg_get_compl_channel(const struct rpma_conn_cfg *cfg,
		bool *shared);
 ...
 *
 * SEE ALSO
 * rpma_conn_cfg_new(3), rpma_conn_wait(3), rpma_conn_cfg_get_compl_channel(3),
 * librpma(7) and https://pmem.io/rpma/
 */
 ...
 *
 * Note that rpma_srq_new(3) does not create its own receive CQ
 * if the size of the receive CQ equals 0.
 *
 * RETURN VALUE
 ...
 *
 * DESCRIPTION
 * rpma_srq_new() creates a new shared RQ object including a new shared RQ
 * and a new shared receive CQ. It does not create the shared receive CQ
 ...
 *	struct rpma_srq;
 *	struct rpma_mr_local;
 *	int rpma_srq_recv(struct rpma_srq *srq,
 *			struct rpma_mr_local *dst, size_t offset,
 *			size_t len, const void *op_context);
 *
 * DESCRIPTION
 ...
 * a message arrives it is placed in one of the buffers awaitaning and
 ...
 *	struct rpma_srq;
 *	struct rpma_cq;
 *	int rpma_srq_get_rcq(const struct rpma_srq *srq,
 *			struct rpma_cq **rcq_ptr);
 *
 * DESCRIPTION
 ...
 * rpma_srq_recv(3) completions within the SRQ. rpma_srq_cfg_set_rcq_size(3)
