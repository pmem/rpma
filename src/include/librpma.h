...
#define RPMA_CFG_ON	1
#define RPMA_CFG_OFF	0
...
/** 3
 * rpma_peer_cfg_set_ddio - set the value of the ddio field
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 *
 *	int rpma_peer_cfg_set_ddio(struct rpma_peer_cfg *pcfg, int ddio);
 *
 * ERRORS
...
/** 3
 * rpma_peer_cfg_set_auto_flush - set the value of the auto flush field
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 ...
 * - XXX
 */
int rpma_peer_cfg_set_auto_flush(struct rpma_peer_cfg *pcfg, int auto_flush);
 ...
 *			struct rpma_peer_cfg *pcfg, int *support);
 *
 * ERRORS
...
/* The number of bytes required to store a description of a peer descriptor */
#define RPMA_PEER_CFG_DESCRIPTOR_SIZE 21
...
/** 3
 * rpma_conn_set_remote_peer_cfg - set remote peer cfg for the connection
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 *
 *	int rpma_conn_set_remote_peer_cfg(struct rpma_conn *conn,
 *			struct rpma_peer_cfg *pcfg);
 *
 * ERRORS
 * rpma_conn_set_remote_peer_cfg() can fail with the following error:
 *
 * - XXX
 */
int rpma_conn_set_remote_peer_cfg(struct rpma_conn *conn,
