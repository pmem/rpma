 ...
 *
 * DESCRIPTION
 * rpma_cq_get_wc() receives num_entries completions of already posted
 * operations from the specified CQ to the wc which is a pointer to an array
 * of ibv_wc structs. It receivces all completions if the number of entires
 * in the specified CQ is fewer than num_entries. All operations generate
 * completions on error. The operations posted with the RPMA_F_COMPLETION_ALWAYS
 * flag also generate completions on success. Please see ibv_poll_cq(3)'s
 * manual for details about struct ibv_wc.
 *
 * Note that if the provided cq is the main CQ and the receive CQ is present
 * on the same connection this function won't return RPMA_OP_RECV and
 * RPMA_OP_RECV_RDMA_WITH_IMM at any time. The receive CQ has to be used
 ...
 *
 * RETURN VALUE
 * The rpma_cq_get_wc() function returns a positive value equal to the number
 * of found completions on success or a negative error code on failure.
 * On success, it writes all found completions to the wc. If the status
 ...
 *
 * - RPMA_E_INVAL - num_entries < 1, cq or wc is NULL
 * - RPMA_E_NO_COMPLETION - no completions available
 * - RPMA_E_PROVIDER - ibv_poll_cq(3) failed with a provider error
 * - RPMA_E_UNKNOWN - ibv_poll_cq(3) failed but no provider error is available
 *
