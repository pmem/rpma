 ...
 *
 * DESCRIPTION
 * rpma_cq_get_fd() gets the file descriptor of the CQ. When a next completion
 * in the CQ is ready to be consumed by rpma_cq_get_completion(3),
 * the notification is delivered to a userspace process via the file descriptor.
 * The default mode of the file descriptor is blocking but it can be changed
 * to non-blocking mode using fcntl(2). The CQ is either the connection's
