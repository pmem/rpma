 ...
 * rpma_cq_get_completion(3), the notification is delivered to a userspace
 * process via the file descriptor. The default mode of the file descriptor
 * is blocking but it can be changed to non-blocking mode using fcntl(2).
 * The CQ is either the connection's main CQ or the receive CQ, please see
 * rpma_conn_get_cq(3) and rpma_conn_get_rcq(3) for details.
 *
