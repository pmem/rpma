 ...
 *
 * THREAD SAFETY
 * Most of the librpma API calls are thread-safe but there are also very
 * important exemptions. Here you can find a complete list of NOT thread-safe
 * API calls:
 *
 ...
 *
 * Other librpma API calls are thread-safe. However, creating resources usually
 * involves dynamic memory allocation and destroying resources usually involves
 * a dynamic memory release. The same resource cannot be destroyed more than
 * once, at any thread, and a resource cannot be used after it was destroyed.
 * It is up to the user to follow those rules and not doing so may result in a
 * segmentation fault.
 *
