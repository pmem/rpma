...
/** 3
 * rpma_write_atomic_cmp_swp - initiate the RDMA Compare & Swap operation
 *
 * SYNOPSIS
 *
 *	#include <librpma.h>
 ...
 *	struct rpma_mr_local;
 *	struct rpma_mr_remote;
 *	int rpma_write_atomic_cmp_swp(struct rpma_conn *conn,
 *			struct rpma_mr_remote *swap_dst,
 *			size_t swap_dst_offset,
 *			struct rpma_mr_local *read_dst,
 *			size_t read_dst_offset,
 *			int flags, uint64_t compare, uint64_t swap,
 *			const void *op_context);
 *
 * DESCRIPTION
 * rpma_write_atomic_cmp_swp() initiates the RDMA Compare & Swap operation
 * (transferring data from the local memory to the remote memory). The atomic
 * operation allows reading, comparing and swapping 8 bytes of data atomically
 ...
 *
 * RETURN VALUE
 * The rpma_write_atomic_cmp_swp() function returns 0 on success or a negative
 * error code on failure.
 *
 * ERRORS
 * rpma_write_atomic_cmp_swp() can fail with the following errors:
 *
 * - RPMA_E_INVAL - conn, swap_dst or read_dst is NULL
 * - RPMA_E_INVAL - swap_dst_offset is not aligned to 8 bytes
 * - RPMA_E_INVAL - flags are not set
 * - RPMA_E_PROVIDER - ibv_post_send(3) failed
 *
 * SEE ALSO
