 ...
 * of a connection request object base on obtained request.
 * If succeeds it returns the newly created object.
 */
int
rpma_ep_next_conn_req(struct rpma_ep *ep, struct rpma_conn_req **req)
{
	...
	return rpma_conn_req_from_cm_event(ep->peer, event, req);
}
