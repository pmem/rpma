...
rpma_mr_get_descriptor(struct rpma_mr_local *mr, void *desc)
{
	...
	/* make sure mr->usage can be stored in uint8_t type */
	if (mr->usage > 0xFF) {
		RPMA_LOG_FATAL("value of memory usage too big");
		return RPMA_E_INVAL;
	}
...
/*
 * rpma_mr_remote_supports_persistent_flush -- check if the remote node
 * supports flushing to persistency
 */
int
rpma_mr_remote_supports_persistent_flush(struct rpma_mr_remote *mr)
{
	return (mr->usage & RPMA_MR_USAGE_FLUSH_TYPE_PERSISTENT) != 0;
}
