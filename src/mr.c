		...
		size_t desc_size, struct rpma_mr_remote **mr_ptr)
{
	...
	size_t full_size = sizeof(raddr) + sizeof(size) + sizeof(rkey);

	if (desc_size != full_size) {
		RPMA_LOG_ERROR(
			"incompatible size : %i != %i",
			desc_size, full_size);
		return RPMA_E_INVAL;
	}
...
/*
 * rpma_mr_get_descriptor_size -- get a descriptor size of memory region
 */
int
rpma_mr_get_descriptor_size(struct rpma_mr_local *mr,
		void *desc, size_t *desc_size)
{
	if (mr == NULL || desc == NULL)
		return RPMA_E_INVAL;

	*desc_size = (2 * sizeof(uint64_t)) + sizeof(uint32_t);
