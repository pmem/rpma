...
/*
 * rpma_mr_atomic_cmp_swp -- post an RDMA atomic to swap_dst
 */
int
	...
	int flags, uint64_t compare, uint64_t swap, const void *op_context)
{
	...
	/* read destination */
	sge.addr = (uint64_t)((uintptr_t)read_dst->ibv_mr->addr +
			read_dst_offset);
	sge.length = RPMA_ATOMIC_WRITE_ALIGNMENT;
	sge.lkey = read_dst->ibv_mr->lkey;
	wr.sg_list = &sge;
	wr.num_sge = 1;

	/* swap destination */
	wr.wr.atomic.remote_addr = swap_dst->raddr + swap_dst_offset;
	wr.wr.atomic.compare_add = compare;
	wr.wr.atomic.swap = swap;
	wr.wr.atomic.rkey = swap_dst->rkey;
	...
	wr.send_flags = (flags & RPMA_F_COMPLETION_ON_SUCCESS) ?
		IBV_SEND_SIGNALED : 0;
	/*
	 * the fence is required to simulate the future atomic
	 * write semantics which will wait for the previous flushes
	 * to complete before being executed
	 */
