...
static int rpma_flush_apm_request(struct ibv_qp *qp, struct rpma_flush *flush,
...
rpma_flush_apm_new(struct rpma_peer *peer, struct rpma_flush *flush)
{
	...
	flush_internal->flush_func = rpma_flush_apm_request;
...
/*
 * rpma_flush_apm_request -- perform the APM-style flush
 */
static int
rpma_flush_apm_request(struct ibv_qp *qp, struct rpma_flush *flush,
	struct rpma_mr_remote *dst, size_t dst_offset, size_t len,
	enum rpma_flush_type type, int flags, const void *op_context)
{
	RPMA_DEBUG_TRACE;
