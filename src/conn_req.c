		...
		const struct rpma_conn_cfg *cfg, struct rpma_conn_req **req_ptr)
{
	...
	ret = rpma_cq_create(id->verbs, cqe, &cq);
	if (ret)
		return ret;
 ...
 * rdma_disconnect()+rdma_destroy_qp()+rpma_cq_destroy() to destroy
 ...
 * rdma_destroy_qp()+rpma_cq_destroy()+rdma_destroy_id() to destroy
 * the unsuccessful connection request.
 *
 * ASSUMPTIONS
 * - req != NULL && conn_param != NULL && conn_ptr != NULL
