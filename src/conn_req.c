 ...
 *                      It uses rdma_create_id, rpma_info_resolve_addr
 *                      and rdma_resolve_route and feeds the prepared ID
 *                      into rpma_conn_req_from_id.
 */
int
		...
		struct rpma_conn_req **req_ptr)
{
	...
	/* resolve route */
	ret = rdma_resolve_route(id, RPMA_DEFAULT_TIMEOUT);
	if (ret) {
		Rpma_provider_error = errno;
		ret = RPMA_E_PROVIDER;
		goto err_destroy_id;
