		...
		const struct rpma_conn_cfg *cfg, struct rpma_conn_req **req_ptr)
{
	...
	if (shared && rcqe) {
		ret = rpma_ibv_comp_channel_create(id->verbs, &channel);
		if (ret)
			return ret;
	}
...
err_comp_channel_destroy:
	if (channel)
		(void) rpma_ibv_comp_channel_destroy(channel);
...
err_conn_req_delete:
	...
	if (req->channel)
		(void) rpma_ibv_comp_channel_destroy(req->channel);
...
rpma_conn_req_delete(struct rpma_conn_req **req_ptr)
{
	...
	if (req->channel) {
		int ret2 = rpma_ibv_comp_channel_destroy(req->channel);
		if (ret2 && !ret)
			ret = ret2;
	}
