...
/*
 * ERRORS
 * rpma_ibv_comp_channel_create() can fail with the following errors:
 *
 * - RPMA_E_PROVIDER - ibv_create_comp_channel(3) failed with a provider error
 */
int rpma_ibv_comp_channel_create(struct ibv_context *ibv_ctx,
		struct ibv_comp_channel **channel_ptr);

/*
 * ERRORS
 * rpma_ibv_comp_channel_destroy() can fail with the following errors:
 *
 * - RPMA_E_PROVIDER - ibv_destroy_comp_channel(3) failed with a provider error
 */
int rpma_ibv_comp_channel_destroy(struct ibv_comp_channel *channel);
