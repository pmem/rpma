...
| CMAKE_INSTALL_PREFIX | Install path prefix, prepended onto install directories | /usr/local | /usr/local |
| COVERAGE | Run coverage test | ON/OFF | OFF |
| DEVELOPER_MODE | Enable developer checks | ON/OFF | OFF |
| GROFF | Path to a program | /usr/bin/groff | /usr/bin/groff |
...
| TEST_DIR | Working directory for tests | /rpma/build/test | ./build/test |
| TRACE_TESTS | More verbose test outputs | ON/OFF | OFF |
| TXT2MAN | Path to a program | /usr/local/bin/txt2man | /usr/local/bin/txt2man |
